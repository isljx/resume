    ...
    $.selectPicker = function(el,options){
        ...
        var methods = {
            init:function(){
                ...
                index<0 ? publicMethod.reset() : publicMethod.setByIndex(index);
            },
            htmlBuild:{
                setup : function(){
...
//处理select完成，保存索引
            ...
            toggle : function(){
                ...
                vars.ulContainer.is(":hidden")? methods.open():methods.close();
            },
            close:function(){
                vars.ulContainer.removeClass("active");
                if(vars.trigger){vars.trigger.removeClass("active")}
            },
            open:function(){
                vars.ulContainer.addClass("active");
                if(vars.trigger){vars.trigger.addClass("active")}
            },
            updateStyle:function(obj){
            ...
            templateEngine:function (template,data){
                ...
                function add(substr,isQuotes){
                    substr?(substr = substr.replace('\'','\\"'),substr.match(reExp)?(html += substr):(isQuotes?(  html +='tpl+=\''+substr+'\';'):(  html +='tpl+='+ substr+';'))):(html += "")
                }
                ...
                html = html.replace(/[\r\t\n]/g, '')
                try{
                    return new Function(html).apply(data);
                }catch(e){
        ...
        var publicMethod = {
            ...
            setByIndex:function(index){
                var list = vars.ulContainer.find("li");
               (index<list.length && index >= 0) && methods.setSelected.apply(list.eq(index));
            },
            ...
            click:function(callback){
                ...
                vars.selectWrapper.on("click",".display",function(e){
                    callback&&(typeof callback == "function")&&callback.apply(that,[e]);
                })
            },
            ...
            trigger:function(event){
                var index = publicMethod.index();
                switch(event){
                    case "change":index>=0 && vars.ulContainer.find("li").eq(index).mousedown();
                }
            },
            getName:function(){
                if(vars.isComplete){
    ...
    $.fn.selectPicker = function(options){
        ...
        if(typeof options == "object"){
            $(this).each(function(){
                ($(this).data("selectPicker") === undefined)&&($.selectPicker(this,options));
            })
        }else{
            ...
            if(typeof options == "string"){
                if(methodWithReturn.search(options)>=0){
                    ...
                    switch (options){
                        case "val":return selectPicker.val();
                        case "text":return selectPicker.text();
                        case "index":return selectPicker.index();
                        case "isSelected":return selectPicker.isSelected();
                        case "name":return selectPicker.getName();
                    }
                }else{
                    var argus = arguments;
                    $(this).each(function(){
                        ...
                        if(selectPicker === undefined) return;
                        switch (options){
                            case "refresh":selectPicker.refresh();break;
                            case "click":selectPicker.click.apply(this,[argus[1]]);break;
                            case "blur":selectPicker.blur.apply(this,[argus[1]]);break;
                            ...
                            case "trigger":selectPicker.trigger.apply(this,[argus[1]]);break;
                            case "disabled":selectPicker.disabled.apply(this);break
                            case "enabled":selectPicker.enabled.apply(this);break
                            case "readOnly":selectPicker.readOnly.apply(this);break
                        }
                    })
                }
            }
